...
\draw (ST1.bpin 94) ++(\padding, 0)
    ...
    node[left] at (DECODER.bpin 89) {jump\_index}
    node[left] at (DECODER.bpin 88) {next\_index}
...
\draw
    ...
    (muxRAM.blpin 1) -| ([shift={(-\ps, 0)}] muxRAM.blpin 1 |- DECODER.bpin 88) node[circ] (nextindex) {} -- (DECODER.bpin 88)
;
...
\draw
    ...
    (muxCACHE.blpin 1)
        ...
        -| ([shift={(2*\ps, 0)}] RAM.bpin 12 -| ALU.brpin 1) -- (RAM.bpin 12)
...
% IP
\draw (ST4.bpin 94) ++(\padding + \ps, 0)
    node[dipchip, anchor=bpin 1, num pins=16] (IP) {}
    node[below] at (IP.n) {\normalsize INDEX MANAGER}
    node[right] at (IP.bpin 3) {last\_index}
    node[right] at (IP.bpin 4) {jump}
    node[right] at (IP.bpin 5) {conditions}
    node[right] at (IP.bpin 6) {success\_index}
    node[right] at (IP.bpin 7) {failure\_index}
    node[right] at (IP.bpin 8) {flags}
    node[left] at (IP.bpin 14) {index}
;
\draw
    ...
    (IP.bpin 7) -- (nextindex)
    (IP.bpin 8) -- ++(-\ps, 0) |- (ALU.bbpin 2)
;

% ST5
\draw (IP.bpin 16) ++(\padding, 0)
    node[stage, anchor=bpin 1, num pins=94] (ST5) {}
    node[below] at (ST5.n) {\normalsize 5}
;
\draw
    (IP.bpin 14) -- (ST5.bpin 92) -- ++(\padding, 0) |- (index2)
;
\end{circuitikz}
\end{document}
